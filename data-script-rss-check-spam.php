...
class DataXscript extends \phpcrud\Data{
  ...
  function fn_run_script_owner(){
    ...
    $this->int_id_customer=0;
    $this->str_id_ignore="0";
    $this->int_interval=10000;

    //$str_sql="SELECT id FROM `control`.`support-check` where int_check=0 and id<>$this->int_id_customer and id not in($this->str_id_ignore)and id>202008 order by id desc;";
  ...
  function fn_action_customer($row){
    ...
    $int_status=1;
    if($bln_exist){
      //$str_sql="SELECT * FROM `".$str_name_schema."`.`contact` where email in ('aaabbott52@hotmail.com','alccla@yahoo.com','andujarw78@yahoo.com','alccla@yahoo.com','kviki@verizon.net','susanluethje@gmail.com');";
      $str_sql="SELECT * FROM `".$str_name_schema."`.`contact` where email in ('naturescounter@bigpond.com','aaabbott52@hotmail.com','alccla@yahoo.com','andujarw78@yahoo.com','alccla@yahoo.com','kviki@verizon.net','susanluethje@gmail.com');";
