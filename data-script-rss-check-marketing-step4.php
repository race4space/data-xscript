...
class DataXscript extends \phpcrud\Data{
  ...
  function fn_run_script_owner(){
    ...
    $str_sql="SELECT id FROM `control`.`support-check` where int_check=3 and id not in($this->str_id_ignore) order by id desc;";
  ...
  function  fn_get_sql($int_id_start, $int_id_end){
    $str_sql="SELECT id, companyname, int_check FROM `control`.`support-check` where int_check=3 and (id>=".$int_id_end." and id<=".$int_id_start.")  and id not in($this->str_id_ignore) order by id desc;";
    return $str_sql;
  }
  ...
  function fn_action_customer($row){
    ...
    if($bln_exist){
      ...
      $str_sql_update="DELETE FROM   `".$str_name_schema."`.additionalbutton where displayname='campaigns' and AdditionalButtonType=2 and AdditionalButtonURL='/simplecampaigns/new';";      
